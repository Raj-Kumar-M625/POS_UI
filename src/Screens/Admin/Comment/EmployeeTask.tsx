...
export const EmployeeTask = () => {
  ...
  const { isLoading } = useQuery("EmployeeTask", async () => {
    const comments: any = await Get("app/EmployeeTask/GetComments");
  ...
  const columns: any = [
    {
      field: "employeeTaskId",
      name: "Employee Task Id",
      width: 324,
      rigth: true,
      selector: (row: any) => (
        <p className="tableStyle">{row.employeeTaskId}</p>
      ),
    },
    {
      field: "employee",
